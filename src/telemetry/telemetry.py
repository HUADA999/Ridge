...
def v1_telemetry(telemetry_data: List[Dict[str, str]]):
    ...
    # POST request to new ridge telemetry server
    try:
        requests.post("https://telemetry.ridge.dev/v1/telemetry", json=telemetry_data)
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail="Could not POST equest to new ridge telemetry server. Contact developer to get this fixed.",
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Start Ridge Telemetry Server")
