...
async def aget_or_create_user_by_email(email: str) -> tuple[RidgeUser, bool]:
...
async def aget_user_validated_by_email_verification_code(code: str) -> RidgeUser:
    user = await RidgeUser.objects.filter(email_verification_code=code).afirst()
    if not user:
        return None
...
async def set_text_content_config(user: RidgeUser, object: Type[models.Model], updated_config):
...
class DataStoreAdapters:
    @staticmethod
    async def astore_data(data: dict, key: str, user: RidgeUser, private: bool = True):
...
class ConversationAdapters:
    ...
    @staticmethod
    async def aget_conversation_config(user: RidgeUser):
    ...
    @staticmethod
    def get_default_conversation_config(user: RidgeUser = None):
    ...
    @staticmethod
    async def aget_default_conversation_config(user: RidgeUser = None):
        """Get default conversation config. Prefer chat model by server admin > user > first created chat model"""
        # Get the server chat settings
    ...
    @staticmethod
    def get_advanced_conversation_config(user: RidgeUser):
        server_chat_settings = ServerChatSettings.objects.first()
    ...
    @staticmethod
    async def aget_advanced_conversation_config(user: RidgeUser = None):
        server_chat_settings: ServerChatSettings = (
            await ServerChatSettings.objects.filter()
    ...
    @staticmethod
    def set_conversation_processor_config(user: RidgeUser, new_config: ChatModelOptions):
    ...
    @staticmethod
    async def aget_user_conversation_config(user: RidgeUser):
        config = (
            await UserConversationConfig.objects.filter(user=user).prefetch_related("setting__openai_config").afirst()
        )
        if not config:
    ...
    @staticmethod
    def get_valid_conversation_config(user: RidgeUser, conversation: Conversation):
...
class FileObjectAdapters:
    ...
    @staticmethod
    @arequire_valid_user
    async def aget_all_file_objects(user: RidgeUser):
