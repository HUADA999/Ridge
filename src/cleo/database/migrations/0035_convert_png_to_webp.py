...
def convert_png_images_to_webp(apps, schema_editor):
    ...
    for conversation in Conversations.objects.all():
        for chat in conversation.conversation_log["chat"]:
            if chat["by"] == "ridge" and chat["intent"]["type"] == ImageIntentType.TEXT_TO_IMAGE.value:
            ...
            if chat["by"] == "ridge" and chat["intent"]["type"] == ImageIntentType.TEXT_TO_IMAGE2.value:
                print("❗️ Please MANUALLY update PNG images created by Ridge in your AWS S3 bucket to WebP format.")
...
def convert_webp_images_to_png(apps, schema_editor):
    ...
    for conversation in Conversations.objects.all():
        for chat in conversation.conversation_log["chat"]:
            if chat["by"] == "ridge" and chat["intent"]["type"] == ImageIntentType.TEXT_TO_IMAGE_V3.value:
                # Decode the base64 encoded PNG image
                decoded_image = base64.b64decode(chat["message"])
            ...
            if chat["by"] == "ridge" and chat["intent"]["type"] == ImageIntentType.TEXT_TO_IMAGE2.value:
                ...
                print("❗️ Please MANUALLY update WebP images created by Ridge in your AWS S3 bucket to PNG format.")
