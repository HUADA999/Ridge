...
# Internal Packages
from ridge.configure import configure_routes, initialize_server
...
def run():
    ...
    if not args.gui:
        ...
        from ridge.interface.desktop.main_window import MainWindow, ServerThread
        ...
        logger.info(f"🌗 Ridge is running at {url}")
        try:
            startup_url = url if args.config else f"{url}/config"
            webbrowser.open(startup_url)
        except:
            logger.warning(f"🚧 Unable to open browser. Please open {url} manually to configure or use Ridge.")
        ...
        if system() != "Darwin":
            try:
                ...
                pyi_splash.update_text("Ridge setup complete")
                # Close Splash Screen
                pyi_splash.close()
            except:
                pass
