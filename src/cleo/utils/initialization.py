...
from ridge.utils.constants import (
...
def initialization(interactive: bool = True):
    def _create_admin_user():
        ...
        if not interactive and (not os.getenv("RIDGE_ADMIN_EMAIL") or not os.getenv("RIDGE_ADMIN_PASSWORD")):
            logger.error(
                "🚨 Admin user cannot be created. Please set the RIDGE_ADMIN_EMAIL, RIDGE_ADMIN_PASSWORD environment variables or start server in interactive mode."
            )
            exit(1)
        email_addr = os.getenv("RIDGE_ADMIN_EMAIL") or input("Email: ")
        password = os.getenv("RIDGE_ADMIN_PASSWORD") or input("Password: ")
        admin_user = RidgeUser.objects.create_superuser(email=email_addr, username=email_addr, password=password)
    ...
    admin_user = RidgeUser.objects.filter(is_staff=True).first()
    ...
    chat_config = ConversationAdapters.get_default_chat_model()
    env_default_chat_model = os.getenv("RIDGE_DEFAULT_CHAT_MODEL")
