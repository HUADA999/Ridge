...
from ridge.utils.rawconfig import FileAttachment
...
def message_to_log(
    ...
    ridge_message_metadata = {k: v for k, v in ridge_message_metadata.items() if v is not None}
    ...
    ridge_log = merge_dicts({"message": chat_response, "by": "ridge", "created": ridge_response_time}, ridge_log)
...
def save_to_conversation_log(
    ...
    if generated_excalidraw_diagram:
        ridge_message_metadata["excalidrawDiagram"] = generated_excalidraw_diagram

    updated_conversation = message_to_log(
        user_message=q,
        chat_response=chat_response,
        user_message_metadata=user_message_metadata,
        ridge_message_metadata=ridge_message_metadata,
...
def generate_chatml_messages_with_context(
    ...
    for chat in conversation_log.get("chat", []):
        ...
        if chat["by"] == "ridge" and "excalidraw" in chat["intent"].get("type", ""):
...
def truncate_messages(
    ...
    try:
        ...
        logger.debug(
            f"Fallback to default chat model tokenizer: {default_tokenizer}.\nConfigure tokenizer for model: {model_name} in Ridge settings to improve context stuffing."
        )
...
def commit_conversation_trace(
    ...
    try:
        ...
        if not repo.config_reader().has_option("user", "email"):
            ...
            repo.config_writer().set_value("user", "email", "promptracer@ridge.dev").release()
