...
Ridge: "{inferred_queries if intent_type == "text-to-image" else chat_response}"
""".strip()
    )
...
def generate_chatml_messages_with_context(
    ...
    try:
        ...
        logger.warning(
            f"Fallback to default prompt size: {max_prompt_size}.\nConfigure max_prompt_size for unsupported model: {model_name} in Ridge settings to longer context window."
        )
...
def truncate_messages(
    ...
    try:
        ...
        logger.warning(
            f"Fallback to default chat model tokenizer: {default_tokenizer}.\nConfigure tokenizer for unsupported model: {model_name} in Ridge settings to improve context stuffing."
        )
