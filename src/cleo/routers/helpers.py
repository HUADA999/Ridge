...
class ApiUserRateLimiter:
    ...
    def __call__(self, request: Request):
        ...
        if not subscribed and count_requests >= self.requests:
            ...
            raise HTTPException(
                status_code=429,
                detail="We're glad you're enjoying Ridge! You've exceeded your usage limit for today. Come back tomorrow or subscribe to increase your rate limit via [your settings](https://app.ridge.dev/config).",
            )
...
class ConversationCommandRateLimiter:
    ...
    async def update_and_check_if_valid(self, request: Request, conversation_command: ConversationCommand):
        ...
        if not subscribed and count_requests >= self.trial_rate_limit:
            raise HTTPException(
                status_code=429,
                detail=f"We're glad you're enjoying Ridge! You've exceeded your `/{conversation_command.value}` command usage limit for today. You can increase your rate limit via [your settings](https://app.ridge.dev/config).",
            )
