...
from ridge.processor.conversation.helpers import send_message_to_model_wrapper
...
def construct_chat_history(conversation_history: dict, n: int = 4, agent_name="AI") -> str:
    ...
    for chat in conversation_history.get("chat", [])[-n:]:
        if chat["by"] == "ridge" and chat["intent"].get("type") in ["remember", "reminder", "summarize"]:
            chat_history += f"User: {chat['intent']['query']}\n"
            chat_history += f"{agent_name}: {chat['message']}\n"
        elif chat["by"] == "ridge" and ("text-to-image" in chat["intent"].get("type")):
            chat_history += f"User: {chat['intent']['query']}\n"
            chat_history += f"{agent_name}: [generated image redacted for space]\n"
        elif chat["by"] == "ridge" and ("excalidraw" in chat["intent"].get("type")):
            chat_history += f"User: {chat['intent']['query']}\n"
...
async def schedule_query(
    q: str, conversation_history: dict, user: RidgeUser, query_images: List[str] = None
) -> Tuple[str, ...]:
    """
...
async def extract_relevant_info(
    qs: set[str], corpus: str, user: RidgeUser = None, agent: Agent = None
) -> Union[str, None]:
    """
...
class ApiUserRateLimiter:
    ...
    def __call__(self, request: Request):
        ...
        if not subscribed and count_requests >= self.requests:
            ...
            raise HTTPException(
                status_code=429,
                detail="I'm glad you're enjoying interacting with me! But you've exceeded your usage limit for today. Come back tomorrow or subscribe to increase your usage limit via [your settings](https://app.ridge.dev/settings).",
            )
...
async def create_automation(
    q: str, timezone: str, user: RidgeUser, calling_url: URL, meta_log: dict = {}, conversation_id: str = None
):
