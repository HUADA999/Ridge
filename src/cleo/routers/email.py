...
async def send_magic_link_email(email, unique_id, host):
    ...
    resend.Emails.send(
        {"sender": "noreply@ridge.dev", "to": email, "subject": "Your Sign-In Link for Ridge 🚀", "html": html_content}
    )
...
async def send_query_feedback(uquery, kquery, sentiment, user_email):
    if not is_resend_enabled():
        logger.debug(f"Sentiment: {sentiment}, Query: {uquery}, Ridge Response: {kquery}")
        return
