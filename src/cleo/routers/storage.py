...
# S3 supports serving assets via your domain. Ridge expects this to be used in production. To enable it:
# 1. Your bucket name for images should be of the form sub.domain.tld. For example, generated.ridge.dev
# 2. Add CNAME entry to your domain's DNS records pointing to the S3 bucket. For example, CNAME generated.ridge.dev generated-ridge-dev.s3.amazonaws.com
AWS_RIDGE_IMAGES_BUCKET_NAME = os.getenv("AWS_IMAGE_UPLOAD_BUCKET")
...
def upload_generated_image_to_bucket(image: bytes, user_id: uuid.UUID):
    """Upload ridge generated image to an S3 bucket"""
    return upload_image_to_bucket(
        webp_image=image,
        user_id=user_id,
        bucket_name=AWS_RIDGE_IMAGES_BUCKET_NAME,
    )
