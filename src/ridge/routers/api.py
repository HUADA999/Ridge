...
# Internal Packages
from ridge.configure import configure_processor, configure_server
...
from ridge.utils.config import TextSearchModel
...
from ridge.processor.conversation.prompts import help_message
...
from database.models import LocalMarkdownConfig, LocalOrgConfig, LocalPdfConfig, LocalPlaintextConfig, RidgeUser
...
async def map_config_to_db(config: FullConfig, user: RidgeUser):
...
async def chat(
    ...
    if conversation_command == ConversationCommand.Help:
        ...
        formatted_help = help_message.format(model=model_type, version=state.ridge_version)
