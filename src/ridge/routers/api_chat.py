...
from ridge.app.settings import ALLOWED_HOSTS
...
from ridge.processor.conversation.prompts import (
    help_message,
    no_entries_found,
    no_notes_found,
)
...
from ridge.utils.rawconfig import FilterRequest, LocationData
                ...
                if conversation_commands == [ConversationCommand.Help]:
                    if not q:
                        ...
                        formatted_help = help_message.format(
                            model=model_type, version=state.ridge_version, device=get_device()
                        )
                        async for result in send_llm_response(formatted_help):
                            yield result
                        return
                    custom_filters.append("site:ridge.dev")
    ...
    if conversation_commands == [ConversationCommand.Help]:
        ...
        if q.strip() == help_str:
            ...
            if conversation_config == None:
                conversation_config = await ConversationAdapters.aget_default_conversation_config()
            model_type = conversation_config.model_type
            formatted_help = help_message.format(model=model_type, version=state.ridge_version, device=get_device())
            return StreamingResponse(iter([formatted_help]), media_type="text/event-stream", status_code=200)
        # Adding specification to search online specifically on ridge.dev pages.
        _custom_filters.append("site:ridge.dev")
        conversation_commands.append(ConversationCommand.Online)
