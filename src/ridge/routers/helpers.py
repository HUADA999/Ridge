...
from ridge.processor.content.docx.docx_to_entries import DocxToEntries
from ridge.processor.content.github.github_to_entries import GithubToEntries
from ridge.processor.content.images.image_to_entries import ImageToEntries
...
from ridge.processor.conversation.google.gemini_chat import (
    converse_gemini,
...
from ridge.processor.conversation.openai.gpt import (
    converse_openai,
...
from ridge.processor.speech.text_to_speech import is_eleven_labs_enabled
from ridge.routers.email import is_resend_enabled, send_task_email
...
from ridge.utils.rawconfig import ChatRequestBody, FileAttachment, FileData, LocationData
...
def validate_chat_model(user: RidgeUser):
...
async def is_ready_to_chat(user: RidgeUser):
    ...
    raise HTTPException(status_code=500, detail="Set your OpenAI API key or enable Local LLM via Ridge settings.")
...
async def acreate_title_from_query(query: str, user: RidgeUser = None) -> str:
    """
...
async def acheck_if_safe_prompt(system_prompt: str, user: RidgeUser = None, lax: bool = False) -> Tuple[bool, str]:
    """
...
def schedule_query(
    q: str, conversation_history: dict, user: RidgeUser, query_images: List[str] = None, tracer: dict = {}
) -> Tuple[str, str, str]:
    """
...
async def extract_relevant_info(
    qs: set[str], corpus: str, user: RidgeUser = None, agent: Agent = None, tracer: dict = {}
) -> Union[str, None]:
    """
...
class EmailVerificationApiRateLimiter:
    ...
    async def __call__(self, email: str = None):
        ...
        user: RidgeUser = await aget_user_by_email(email)
        if not user:
...
class ApiUserRateLimiter:
    ...
    def __call__(self, request: Request):
        ...
        user: RidgeUser = request.user.object
        ...
        if not subscribed and count_requests >= self.requests:
            ...
            raise HTTPException(
                status_code=429,
                detail="I'm glad you're enjoying interacting with me! You've unfortunately exceeded your usage limit for today. You can subscribe to increase your usage limit via [your settings](https://app.ridge.dev/settings) or we can continue our conversation tomorrow?",
            )
...
class ConversationCommandRateLimiter:
    ...
    async def update_and_check_if_valid(self, request: Request, conversation_command: ConversationCommand):
        ...
        if not subscribed and count_requests >= self.trial_rate_limit:
            ...
            raise HTTPException(
                status_code=429,
                detail=f"I'm glad you're enjoying interacting with me! You've unfortunately exceeded your `/{conversation_command.value}` command usage limit for today. You can subscribe to increase your usage limit via [your settings](https://app.ridge.dev/settings) or we can talk about something else for today?",
            )
...
def format_automation_response(scheduling_request: str, executed_query: str, ai_response: str, user: RidgeUser) -> bool:
    """
...
def should_notify(original_query: str, executed_query: str, ai_response: str, user: RidgeUser) -> bool:
    """
...
def scheduled_chat(
    ...
    url = f"{scheme}://{calling_url.netloc}/api/chat?client=ridge"

    # Construct the Headers for the chat API
    headers = {"User-Agent": "Ridge", "Content-Type": "application/json"}
    if not state.anonymous_mode:
        # Add authorization request header in non-anonymous mode
        token = get_ridge_tokens(user)
        if is_none_or_empty(token):
            token = create_ridge_token(user).token
        else:
            token = token[0].token
...
def get_user_config(user: RidgeUser, request: Request, is_detailed: bool = False):
