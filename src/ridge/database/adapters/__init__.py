...
def require_valid_user(func: Callable[P, T]) -> Callable[P, T]:
    ...
    def sync_wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
        ...
        user = next((arg for arg in args if isinstance(arg, RidgeUser)), None)
        if not user:
            user = next((val for val in kwargs.values() if isinstance(val, RidgeUser)), None)

        # Throw error if user is not found
        if not user:
            raise ValueError("Ridge user argument required but not provided.")
...
def arequire_valid_user(func: Callable[P, Coroutine[Any, Any, T]]) -> Callable[P, Coroutine[Any, Any, T]]:
    ...
    async def async_wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
        ...
        user = next((arg for arg in args if isinstance(arg, RidgeUser)), None)
        if not user:
            user = next((v for v in kwargs.values() if isinstance(v, RidgeUser)), None)
...
async def aget_or_create_user_by_phone_number(phone_number: str) -> tuple[RidgeUser, bool]:
    is_new = False
...
@arequire_valid_user
async def aset_user_phone_number(user: RidgeUser, phone_number: str) -> RidgeUser:
    if is_none_or_empty(phone_number):
        return None
...
@arequire_valid_user
async def aremove_phone_number(user: RidgeUser) -> RidgeUser:
...
async def acreate_user_by_phone_number(phone_number: str) -> RidgeUser:
    if is_none_or_empty(phone_number):
        return None
    user, _ = await RidgeUser.objects.filter(phone_number=phone_number).aupdate_or_create(
...
async def aget_or_create_user_by_email(input_email: str, check_deliverability=False) -> tuple[RidgeUser, bool]:
    ...
    is_existing_user = await RidgeUser.objects.filter(email=email).aexists()
    ...
    user, is_new = await RidgeUser.objects.filter(email=email).aupdate_or_create(
...
@arequire_valid_user
async def astart_trial_subscription(user: RidgeUser) -> Subscription:
...
async def aget_user_validated_by_email_verification_code(code: str, email: str) -> tuple[Optional[RidgeUser], bool]:
    ...
    user = await RidgeUser.objects.filter(email_verification_code=code, email=normalized_email).afirst()
    if not user:
        return None, False
...
@require_valid_user
def set_user_name(user: RidgeUser, first_name: str, last_name: str) -> RidgeUser:
...
@require_valid_user
def get_user_name(user: RidgeUser):
...
@require_valid_user
def get_user_photo(user: RidgeUser):
    google_profile: GoogleUser = GoogleUser.objects.filter(user=user).first()
    if google_profile:
...
@arequire_valid_user
async def aget_user_subscription_state(user: RidgeUser) -> str:
    """Get subscription state of user
    Valid state transitions: trial -> subscribed <-> unsubscribed OR expired
    """
    user_subscription = await Subscription.objects.filter(user=user).afirst()
...
@arequire_valid_user
async def ais_user_subscribed(user: RidgeUser) -> bool:
    """
...
@require_valid_user
def is_user_subscribed(user: RidgeUser) -> bool:
    """
    Get whether the user is subscribed
    """
    if not state.billing_enabled or state.anonymous_mode:
...
async def aget_user_by_phone_number(phone_number: str) -> RidgeUser:
    if is_none_or_empty(phone_number):
        return None
    matched_user = await RidgeUser.objects.filter(phone_number=phone_number).prefetch_related("subscription").afirst()

    if not matched_user:
        return None

    # If the user with this phone number does not have an email account with Ridge, return the user
    if is_none_or_empty(matched_user.email):
        return matched_user

    # If the user has an email account with Ridge and a verified number, return the user
...
async def retrieve_user(session_id: str) -> RidgeUser:
    ...
    user = await RidgeUser.objects.filter(id=session_data.get("_auth_user_id")).afirst()
    if not user:
        raise HTTPException(status_code=401, detail="Invalid user")
    return user


def get_all_users() -> BaseManager[RidgeUser]:
    return RidgeUser.objects.all()
...
@arequire_valid_user
async def aget_user_name(user: RidgeUser):
    full_name = user.get_full_name()
    if not is_none_or_empty(full_name):
        return full_name
...
@arequire_valid_user
async def set_user_github_config(user: RidgeUser, pat_token: str, repos: list):
...
class AgentAdapters:
    ...
    @staticmethod
    async def aget_readonly_agent_by_slug(agent_slug: str, user: RidgeUser):
        return (
    ...
    @staticmethod
    @arequire_valid_user
    async def adelete_agent_by_slug(agent_slug: str, user: RidgeUser):
    ...
    @staticmethod
    async def aget_agent_by_slug(agent_slug: str, user: RidgeUser):
        return (
            await Agent.objects.filter(
    ...
    @staticmethod
    async def aget_agent_by_name(agent_name: str, user: RidgeUser):
        return (
            await Agent.objects.filter(
    ...
    @staticmethod
    def get_agent_by_slug(slug: str, user: RidgeUser = None):
        if user:
    ...
    @staticmethod
    def get_all_accessible_agents(user: RidgeUser = None):
    ...
    @staticmethod
    async def aget_all_accessible_agents(user: RidgeUser = None) -> List[Agent]:
        agents = await sync_to_async(AgentAdapters.get_all_accessible_agents)(user)
        return await sync_to_async(list)(agents)

    @staticmethod
    async def ais_agent_accessible(agent: Agent, user: RidgeUser) -> bool:
    ...
    @staticmethod
    def create_default_agent(user: RidgeUser):
    ...
    @staticmethod
    def get_agent_chat_model(agent: Agent, user: Optional[RidgeUser]) -> Optional[ChatModel]:
        """
    ...
    @staticmethod
    async def aget_agent_chat_model(agent: Agent, user: Optional[RidgeUser]) -> Optional[ChatModel]:
...
class PublicConversationAdapters:
    ...
    @staticmethod
    def delete_public_conversation_by_slug(user: RidgeUser, slug: str):
...
class ConversationAdapters:
    ...
    def get_conversation_by_user(
        user: RidgeUser, client_application: ClientApplication = None, conversation_id: str = None
    ...
    @staticmethod
    @require_valid_user
    def get_all_conversations_for_export(user: RidgeUser, page: Optional[int] = 0):
        all_conversations = Conversation.objects.filter(user=user).prefetch_related("agent")[page : page + 10]
        histories = []
        for conversation in all_conversations:
            history = {
                "title": conversation.title,
                "agent": conversation.agent.name if conversation.agent else "Ridge",
    ...
    @staticmethod
    @require_valid_user
    def get_num_conversations(user: RidgeUser):
        return Conversation.objects.filter(user=user).count()

    @staticmethod
    @require_valid_user
    def get_conversation_sessions(user: RidgeUser, client_application: ClientApplication = None):
        return (
    ...
    async def aset_conversation_title(
        user: RidgeUser, client_application: ClientApplication, conversation_id: str, title: str
    ):
    ...
    async def acreate_conversation_session(
        user: RidgeUser, client_application: ClientApplication = None, agent_slug: str = None, title: str = None
    ):
        if agent_slug:
    ...
    @staticmethod
    @require_valid_user
    def has_any_chat_model(user: RidgeUser):
    ...
    @staticmethod
    @arequire_valid_user
    async def aset_user_conversation_processor(user: RidgeUser, conversation_processor_config_id: int):
    ...
    @staticmethod
    @arequire_valid_user
    async def aset_user_voice_model(user: RidgeUser, model_id: str):
    ...
    @staticmethod
    def get_chat_model(user: RidgeUser):
    ...
    @staticmethod
    async def aget_chat_model(user: RidgeUser):
    ...
    @staticmethod
    async def aget_voice_model_config(user: RidgeUser) -> Optional[VoiceModelOption]:
    ...
    @staticmethod
    def get_voice_model_config(user: RidgeUser) -> Optional[VoiceModelOption]:
    ...
    @staticmethod
    def get_default_chat_model(user: RidgeUser = None):
    ...
    @staticmethod
    async def aget_default_chat_model(user: RidgeUser = None, fallback_chat_model: Optional[ChatModel] = None):
    ...
    @staticmethod
    def get_advanced_chat_model(user: RidgeUser):
        server_chat_settings = ServerChatSettings.objects.first()
    ...
    @staticmethod
    async def aget_advanced_chat_model(user: RidgeUser = None):
        server_chat_settings: ServerChatSettings = (
            await ServerChatSettings.objects.filter()
    ...
    def create_conversation_from_public_conversation(
        user: RidgeUser, public_conversation: PublicConversation, client_app: ClientApplication
    ):
    ...
    @staticmethod
    def set_user_chat_model(user: RidgeUser, chat_model: ChatModel):
    ...
    @staticmethod
    async def aget_user_chat_model(user: RidgeUser):
        config = (
            await UserConversationConfig.objects.filter(user=user).prefetch_related("setting__ai_model_api").afirst()
        )
        if not config:
    ...
    @staticmethod
    @arequire_valid_user
    async def aget_conversation_starters(user: RidgeUser, max_results=3):
        all_questions = []
    ...
    @staticmethod
    async def aget_valid_chat_model(user: RidgeUser, conversation: Conversation, is_subscribed: bool):
        agent: Agent = (
            conversation.agent
    ...
    @staticmethod
    def get_user_text_to_image_model_config(user: RidgeUser):
    ...
    @staticmethod
    async def aget_user_text_to_image_model(user: RidgeUser) -> Optional[TextToImageModelConfig]:
    ...
    @staticmethod
    async def aset_user_text_to_image_model(user: RidgeUser, text_to_image_model_config_id: int):
    ...
    @staticmethod
    def add_files_to_filter(user: RidgeUser, conversation_id: str, files: List[str]):
    ...
    @staticmethod
    def remove_files_from_filter(user: RidgeUser, conversation_id: str, files: List[str]):
        conversation = ConversationAdapters.get_conversation_by_user(user, conversation_id=conversation_id)
        if not conversation:
            return []
        for filename in files:
    ...
    @staticmethod
    @require_valid_user
    def delete_message_by_turn_id(user: RidgeUser, conversation_id: str, turn_id: str):
        conversation = ConversationAdapters.get_conversation_by_user(user, conversation_id=conversation_id)
...
class EntryAdapters:
    ...
    @staticmethod
    @require_valid_user
    def does_entry_exist(user: RidgeUser, hashed_value: str) -> bool:
        return Entry.objects.filter(user=user, hashed_value=hashed_value).exists()

    @staticmethod
    @require_valid_user
    def delete_entry_by_file(user: RidgeUser, file_path: str):
        deleted_count, _ = Entry.objects.filter(user=user, file_path=file_path).delete()
        return deleted_count

    @staticmethod
    @require_valid_user
    def get_filtered_entries(user: RidgeUser, file_type: str = None, file_source: str = None):
    ...
    @staticmethod
    @require_valid_user
    def delete_all_entries(user: RidgeUser, file_type: str = None, file_source: str = None, batch_size=1000):
        deleted_count = 0
    ...
    @staticmethod
    @arequire_valid_user
    async def adelete_all_entries(user: RidgeUser, file_type: str = None, file_source: str = None, batch_size=1000):
        deleted_count = 0
        queryset = EntryAdapters.get_filtered_entries(user, file_type, file_source)
    ...
    @staticmethod
    @require_valid_user
    def get_existing_entry_hashes_by_file(user: RidgeUser, file_path: str):
        return Entry.objects.filter(user=user, file_path=file_path).values_list("hashed_value", flat=True)

    @staticmethod
    @require_valid_user
    def delete_entry_by_hash(user: RidgeUser, hashed_values: List[str]):
    ...
    @staticmethod
    @arequire_valid_user
    async def auser_has_entries(user: RidgeUser):
    ...
    @staticmethod
    @arequire_valid_user
    async def adelete_entries_by_filenames(user: RidgeUser, filenames: List[str], batch_size=1000):
        deleted_count = 0
    ...
    @staticmethod
    @require_valid_user
    def get_all_filenames_by_source(user: RidgeUser, file_source: str):
        return (
    ...
    @staticmethod
    @require_valid_user
    def get_all_filenames_by_type(user: RidgeUser, file_type: str):
        return (
    ...
    @staticmethod
    @require_valid_user
    def get_size_of_indexed_data_in_mb(user: RidgeUser):
        entries = Entry.objects.filter(user=user).iterator()
        total_size = sum(sys.getsizeof(entry.compiled) for entry in entries)
        return total_size / 1024 / 1024

    @staticmethod
    def apply_filters(user: RidgeUser, query: str, file_type_filter: str = None, agent: Agent = None):
        q_filter_terms = Q()
...
class AutomationAdapters:
    ...
    @staticmethod
    def get_automation_metadata(user: RidgeUser, automation: Job):
    ...
    @staticmethod
    def get_job_last_run(user: RidgeUser, automation: Job):
        # Perform validation checks
        # Check if user is allowed to delete this automation id
        if not automation.id.startswith(f"automation_{user.uuid}_"):
            raise ValueError("Invalid automation id")
    ...
    @staticmethod
    def get_automations_metadata(user: RidgeUser):
    ...
    @staticmethod
    def get_automation(user: RidgeUser, automation_id: str) -> Job:
        # Perform validation checks
    ...
    @staticmethod
    async def aget_automation(user: RidgeUser, automation_id: str) -> Job:
        # Perform validation checks
        # Check if user is allowed to retrieve this automation id
        if is_none_or_empty(automation_id) or not automation_id.startswith(f"automation_{user.uuid}_"):
            raise ValueError("Invalid automation id")
    ...
    @staticmethod
    def delete_automation(user: RidgeUser, automation_id: str):
