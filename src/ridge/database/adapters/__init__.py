...
async def aget_or_create_user_by_phone_number(phone_number: str) -> RidgeUser:
...
async def aget_or_create_user_by_email(email: str) -> RidgeUser:
    user, _ = await RidgeUser.objects.filter(email=email).aupdate_or_create(defaults={"username": email, "email": email})
    await user.asave()
