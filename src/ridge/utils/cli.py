...
from ridge.utils.helpers import in_debug_mode, is_env_var_true, resolve_absolute_path
...
def cli(args=None):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Start Ridge in non-interactive mode. Assumes interactive shell unavailable for config. E.g when run via Docker.",
    )
    ...
    if not args.config_file.exists():
        ...
        if is_env_var_true("RIDGE_TELEMETRY_DISABLE") or in_debug_mode():
