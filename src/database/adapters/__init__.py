...
class EmbeddingsAdapters:
    ...
    @staticmethod
    def does_embedding_exist(user: RidgeUser, hashed_value: str) -> bool:
        return Embeddings.objects.filter(user=user, hashed_value=hashed_value).exists()

    @staticmethod
    def delete_embedding_by_file(user: RidgeUser, file_path: str):
        deleted_count, _ = Embeddings.objects.filter(user=user, file_path=file_path).delete()
        return deleted_count

    @staticmethod
    def delete_all_embeddings(user: RidgeUser, file_type: str):
    ...
    @staticmethod
    def delete_embedding_by_hash(user: RidgeUser, hashed_values: List[str]):
    ...
    @staticmethod
    async def user_has_embeddings(user: RidgeUser):
