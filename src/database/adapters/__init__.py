...
async def set_user_subscription(email: str, is_subscribed=None, renewal_date=None, type="standard") -> RidgeUser:
...
def get_user_subscription_state(email: str) -> str:
    ...
    """
    user = RidgeUser.objects.filter(email=email).first()
    if user.subscription_type == RidgeUser.SubscriptionType.TRIAL:
        return "trial"
