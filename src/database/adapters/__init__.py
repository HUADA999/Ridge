...
async def set_user_unsubscribed(email: str, type="standard") -> RidgeUser:
    user = await RidgeUser.objects.filter(email=email, subscription_type=type).afirst()
    if user:
...
async def set_user_subscribed(email: str, type="standard") -> RidgeUser:
    user = await RidgeUser.objects.filter(email=email).afirst()
    if user:
...
def is_user_subscribed(email: str, type="standard") -> bool:
    return RidgeUser.objects.filter(
...
class EntryAdapters:
    ...
    @staticmethod
    def delete_all_entries_by_source(user: RidgeUser, file_source: str = None):
    ...
    def search_with_embeddings(
        user: RidgeUser, embeddings: Tensor, max_results: int = 10, file_type_filter: str = None, raw_query: str = None
    ):
    ...
    @staticmethod
    def get_unique_file_source(user: RidgeUser):
