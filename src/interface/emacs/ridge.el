...
(defun ridge--get-current-org-entry-text ()
...
(defun ridge--find-similar-notes ()
  "Search for org entries similar to entry at point."
  (let* ((rerank "true")
         (content-type (ridge--buffer-name-to-content-type (buffer-name)))
         (query (ridge--get-current-org-entry-text))
...
(transient-define-suffix ridge--find-similar-command (&optional _)
  "Find other notes similar to current note at point."
  (interactive)
  (ridge--find-similar-notes))
...
(transient-define-prefix ridge-menu ()
  ...
  [["Act"
    ...
    ("f" "Find Similar Notes" ridge--find-similar-command)
