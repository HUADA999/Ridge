...
class ConfigureScreen(QtWidgets.QDialog):
    ...
    def __init__(self, config_file: Path, parent=None):
        ...
        if self.current_config is None:
            self.current_config = yaml_utils.load_config_from_file(constants.app_root_directory / 'config/ridge_sample.yml')
    ...
    def get_default_config(self, search_type:SearchType=None, processor_type:ProcessorType=None):
        "Get default config"
        config = yaml_utils.load_config_from_file(constants.app_root_directory / 'config/ridge_sample.yml')
        if search_type:
