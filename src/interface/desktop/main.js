...
const RIDGE_URL = 'https://app.ridge.dev';

const Store = require('electron-store');

const textFileTypes = [
    // Default valid file extensions supported by Ridge
...
function pushDataToRidge (regenerate = false) {
    ...
    for (const file of filesToPush) {
        ...
        // Collect all updated or newly created files since last sync to index on Ridge server
        try {
    ...
    // Mark deleted files for removal from index on Ridge server
    ...
    // Send collected files to Ridge server for indexing
    ...
    .catch(error => {
        ...
        if (error?.response?.status === 429 && (BrowserWindow.getAllWindows().find(win => win.webContents.getURL().includes('settings')))) {
            state["error"] = `Looks like you're out of space to sync your files. <a href="https://app.ridge.dev/settings#subscription">Upgrade your plan</a> to unlock more space.`;
...
async function getToken () {
    return store.get('ridgeToken');
}

async function setToken (event, token) {
    store.set('ridgeToken', token);
    return store.get('ridgeToken');
}
...
function addCSPHeaderToSession () {
    ...
    const defaultDomains = `'self' ${hostURL} https://app.ridge.dev https://assets.ridge.dev`;
    ...
    const img_src = `img-src ${defaultDomains} data: https://*.ridge.dev https://*.googleusercontent.com;`;
...
app.whenReady().then(() => {
    ...
    app.setAboutPanelOptions({
        applicationName: "Ridge",
        applicationVersion: ridgePackage.version,
        version: ridgePackage.version,
        authors: "Ridge AI",
        website: "https://ridge.dev",
        copyright: "GPL v3",
...
function openAboutWindow() {
    ...
    // Pass OS, Ridge version to About page
    aboutWindow.webContents.on('did-finish-load', () => {
        aboutWindow.webContents.send('appInfo', { version: ridgePackage.version, platform: process.platform });
    });
...
app.whenReady().then(() => {
    ...
    const contextMenu = Menu.buildFromTemplate([
        ...
        { type: 'separator' },
        { label: 'About Ridge', type: 'normal', click: () => { openAboutWindow(); } },
        { label: 'Quit', type: 'normal', click: () => { app.quit() } }
    ])

    tray.setToolTip('Ridge')
