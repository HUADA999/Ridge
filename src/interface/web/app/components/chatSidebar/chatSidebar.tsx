...
function ChatSidebarInternal({ ...props }: ChatSideBarProps) {
    ...
    const [isDefaultAgent, setIsDefaultAgent] = useState<boolean>(agentData?.slug.toLowerCase() === "ridge" ? true : false);
    ...
    function setupAgentData() {
        if (agentData) {
            ...
            if (agentData.name.toLowerCase() === "ridge" || agentData.is_hidden === true) {
                setIsEditable(true);
            }

            if (agentData.slug.toLowerCase() === "ridge") {
