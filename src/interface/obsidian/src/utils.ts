...
export async function updateContentIndex(vault: Vault, setting: RidgeSetting, lastSync: Map<TFile, number>, regenerate: boolean = false, userTriggered: boolean = false): Promise<Map<TFile, number>> {
    ...
    for (const contentType of contentTypesToDelete) {
        const response = await fetch(`${setting.ridgeUrl}/api/content/type/${contentType}?client=obsidian`, {
            method: "DELETE",
            headers: {
    ...
    for (const batch of fileData) {
        ...
        // Call Ridge backend to sync index with updated files in vault
        ...
        if (!response.ok) {
            if (response.status === 429) {
                ...
                error_message = `❗️Failed to sync all your content with Ridge server. Raise issue on Ridge Discord or Github\nError: ${response.statusText}`;
            }
        } else {
