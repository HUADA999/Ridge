...
export class RidgeSearchModal extends SuggestModal<SearchResult> {
    ...
    async getSuggestions(query: string): Promise<SearchResult[]> {
        ...
        try {
            ...
            // Setup Query Ridge backend for search results
            ...
            // Get search results from Ridge backend
    ...
    async renderSuggestion(result: SearchResult & { inVault: boolean }, el: HTMLElement) {
        ...
        const fileEl = el.createEl("div", {
            cls: `ridge-result-file ${result.inVault ? 'in-vault' : 'not-in-vault'}`
        });
