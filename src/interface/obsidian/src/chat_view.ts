...
import { RidgeSearchModal } from './search_modal';
...
export class RidgeChatView extends RidgePaneView {
    ...
    constructor(leaf: WorkspaceLeaf, setting: RidgeSetting) {
        ...
        this.scope.register(["Ctrl"], 'f', (_) => new RidgeSearchModal(this.app, this.setting).open());
        this.scope.register(["Ctrl"], 'r', (_) => new RidgeSearchModal(this.app, this.setting, true).open());
    ...
    async onOpen() {
        ...
        // WARNING: CSP DISABLED for now as it breaks other Obsidian plugins. Enable when can scope CSP to only Ridge plugin.
        // CSP meta tag for the Ridge Chat modal
        ...
        let placeholderText: string = getChatHistorySucessfully ? this.startingMessage : "Configure Ridge to enable chat";
    ...
    textToSpeech(message: string, event: MouseEvent | null = null): void {
        ...
        let textToSpeechApi = `${this.setting.ridgeUrl}/api/chat/speech?text=${encodeURIComponent(message)}`;
    ...
    generateImageMarkdown(message: string, intentType: string, inferredQueries?: string[], conversationId?: string, images?: string[], excalidrawDiagram?: string): string {
        ...
        if (intentType === "text-to-image") {
            ...
            const domain = this.setting.ridgeUrl.endsWith("/") ? this.setting.ridgeUrl : `${this.setting.ridgeUrl}/`;
    ...
    renderMessage({ chatBodyEl, message, sender, dt, turnId, raw = false, willReplace = true, isSystemMessage = false }: RenderMessageOptions): Element {
        ...
        chatMessageBodyEl.addClasses(["ridge-chat-message-text", sender]);
    ...
    renderActionButtons(message: string, chatMessageBodyTextEl: HTMLElement, isSystemMessage: boolean = false) {
        ...
        if (!isSystemMessage) {
            ...
            deleteButton.addEventListener('click', () => {
                const messageEl = chatMessageBodyTextEl.closest('.ridge-chat-message');
                if (messageEl) {
    ...
    createNewConversation() {
        ...
        const chatInput = this.contentEl.querySelector('.ridge-chat-input') as HTMLTextAreaElement;
        if (chatInput) {
            chatInput.placeholder = this.startingMessage;
        }
        this.renderMessage({chatBodyEl, message: "Hey 👋🏾, what's up?", sender: "ridge", isSystemMessage: true});
    }
    ...
    async getChatResponse(query: string | undefined | null, isVoice: boolean = false): Promise<void> {
        ...
        const chatUrl = `${this.setting.ridgeUrl}/api/chat?client=obsidian`;
        const body = {
            q: query,
        ...
        let newResponseEl = this.createRidgeResponseDiv();
        ...
        try {
            ...
            console.error(`Ridge chat response failed with\n${err}`);
    ...
    async deleteMessage(messageEl: HTMLElement) {
        ...
        try {
            const response = await fetch(`${this.setting.ridgeUrl}/api/chat/conversation/message`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${this.setting.ridgeApiKey}`
            ...
            if (response.ok) {
                // Remove both the user message and Ridge response (the conversation turn)
                ...
                } else if (!isRidgeMessage && messageIndex < messages.length - 1) {
                    // If it is a user message, remove the next Ridge message too
                    messages[messageIndex + 1].remove();
                }
                messageEl.remove();
            } else {
