# Admin Panel
> Describes the Ridge settings configurable via the admin panel

By default, your admin panel is available at `http://localhost:42110/server/admin/`. You can access the admin panel by logging in with your admin credentials (this would be your `RIDGE_ADMIN_EMAIL` and `RIDGE_ADMIN_PASSWORD`). The admin panel allows you to configure various settings for your Ridge server.
...
- `Public`: Check this if the agent is expected to be visible to all users on this Ridge server.
...
Add text to image generation models with these settings. Ridge currently supports text to image models available via OpenAI, Stability or Replicate API
...
Add speech to text models with these settings. Ridge currently only supports whisper speech to text model via OpenAI API or Offline

### Voice Model Options
Add text to speech models with these settings. Ridge currently supports models from [ElevenLabs](https://elevenlabs.io/).
...
- Client Applications:
  ...
  Client applications allow you to setup third party applications that can query your Ridge server using a client application ID + secret. The secret would go in a bearer token.
