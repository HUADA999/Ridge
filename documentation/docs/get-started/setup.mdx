...
To use the offline chat model with your GPU, we recommend using the Docker setup with Ollama . You can also use the local Ridge setup via the Python package directly.
:::

:::info[First Run]
Restart your Ridge server after the first run to ensure all settings are applied correctly.
:::
...
Your setup is complete once you see `🌖 Ridge is ready to engage` in the server logs on your terminal.
  </TabItem>
  <TabItem value="pip" label="Pip">

<h3>1. Install Ridge Server</h3>
...
Run the following command in your terminal to install the Ridge server.
  ...
  <Tabs groupId="operating-systems" queryString="os">
    ...
    ```shell
CMAKE_ARGS="-DGGML_METAL=on" python -m pip install 'ridge[local]'
    ```
        </TabItem>
        <TabItem value="intel" label="Intel">
    ```shell
python -m pip install 'ridge[local]'
    ```
        </TabItem>
      </Tabs>
    </TabItem>
 ...
 py -m pip install 'ridge[local]'
    ...
    <TabItem value="linux" label="Linux">
      <Tabs groupId="gpu" queryString="gpu">
        ...
        <TabItem value="nvidia" label="NVIDIA (CUDA) GPU">
          ```shell
          CMAKE_ARGS="-DGGML_CUDA=on" FORCE_CMAKE=1 python -m pip install 'ridge[local]'
          ```
        </TabItem>
        <TabItem value="amd" label="AMD (ROCm) GPU">
          ```shell
          CMAKE_ARGS="-DGGML_HIPBLAS=on" FORCE_CMAKE=1 python -m pip install 'ridge[local]'
          ```
        </TabItem>
        <TabItem value="vulkan" label="VULKAN GPU">
          ```shell
          CMAKE_ARGS="-DGGML_VULKAN=on" FORCE_CMAKE=1 python -m pip install 'ridge[local]'
          ```
        </TabItem>
      </Tabs>
    </TabItem>
...
3. Your setup is complete once you see `🌖 Ridge is ready to engage` in the server logs on your terminal!
...
If using a load balancer/reverse_proxy in front of your Ridge server: Set the environment variable RIDGE_ALLOWED_DOMAIN=your-internal-ip-or-domain to avoid this error.
If unset, it defaults to RIDGE_DOMAIN.
:::
...
Set the environment variables RIDGE_DOMAIN=your-external-ip-or-domain and RIDGE_NO_HTTPS=True if required to avoid this error.
:::
