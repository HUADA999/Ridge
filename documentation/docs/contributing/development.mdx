...
Welcome to the development docs of Ridge! Thanks for your interest in being a contributor ❤️. Open source contributors are a corner-store of the Ridge community. We welcome all contributions, big or small.
...
Ridge uses the `pgvector` package to store embeddings of your index in a Postgres database. To use this, you need to have Postgres installed.
...
1. Command `ridge` Not Recognized
    - Try reactivating the virtual environment and rerunning the `ridge` command.
    - If it still doesn't work repeat the installation process.
2. Python Package Missing
    - Use `pip install xxx` and try running the `ridge` command.
...
:::tip[Note]
You should be in an active virtual environment for Ridge in order to run the unit tests and linter.
:::
...
### After Creating PR
1. Automated [validation workflows](https://github.com/ridge-ai/ridge/tree/master/.github/workflows) should run for every PR. Tag one of the maintainers in the PR to trigger it.
...
Follow the steps below to [release](https://github.com/ridge-ai/ridge/releases/) Ridge. This will create a stable release of Ridge on [Pypi](https://pypi.org/project/ridge/), [Melpa](https://stable.melpa.org/#%252Fridge) and [Obsidian](https://obsidian.md/plugins?id%253Dridge). It will also create desktop apps of Ridge and attach them to the latest release.
...
![](/img/ridge_architecture.png)

## Visualize Codebase

*[Interactive Visualization](https://mango-dune-07a8b7110.1.azurestaticapps.net/?repo=ridge-ai%2Fridge)*

![](/img/ridge_codebase_visualization_0.2.1.png)

## Visualize Ridge Obsidian Plugin Codebase

![](/img/ridge_obsidian_codebase_visualization_0.2.1.png)
