...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate Ridge on the Google FRAMES benchmark.")
    parser.add_argument(
        "--output",
        "-o",
        default=None,
...
if __name__ == "__main__":
    """
    Evaluate Ridge on the Google FRAMES benchmark.
    ...
    """
    logger.info(f"{datetime.now()} - Begin Quizzing Ridge on the FRAMES benchmark.")
    with timer("Ran eval script in", logger, log_level=logging.INFO):
        main()
    logger.info(f"{datetime.now()} - End Quizzing Ridge on the FRAMES benchmark.")
