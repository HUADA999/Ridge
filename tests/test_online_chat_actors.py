...
from ridge.processor.conversation.openai.gpt import converse_openai, extract_questions
...
@pytest.mark.anyio
@pytest.mark.django_db(transaction=True)
async def test_websearch_ridge_website_for_info_about_ridge(chat_client, default_user2):
    # Arrange
    ...
    # Assert
    assert any(
        ["site:ridge.dev" in response for response in responses]
    ), "Expected search query to include site:ridge.dev but got: " + str(responses)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "https://ridge-generated-wallpaper.ridge.dev/user110/weathervane.webp",
            True,
        ),
        (
