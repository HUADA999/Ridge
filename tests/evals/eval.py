...
RIDGE_MODE = os.getenv("RIDGE_MODE", "default").lower()  # E.g research, general, notes etc.

GEMINI_API_KEY = os.getenv("GEMINI_API_KEY")
GEMINI_EVAL_MODEL = os.getenv("GEMINI_EVAL_MODEL", "gemini-2.0-flash-001")

LLM_SEED = int(os.getenv("RIDGE_LLM_SEED")) if os.getenv("RIDGE_LLM_SEED") else None
...
SLEEP_SECONDS = 3 if RIDGE_MODE == "general" else 1  # Sleep between API calls to avoid rate limiting
...
def index_frames_kb():
    """Index Wikipedia articles from FRAMES dataset into Ridge"""
    try:
        # Load dataset
        dataset = load_frames_kb()
        dataset_files = set(map(get_article_filename, dataset))

        # Get indexed files from Ridge API
        headers = {"Authorization": f"Bearer {RIDGE_API_KEY}"} if RIDGE_API_KEY else {}
        try:
            response = requests.get(f"{RIDGE_URL}/api/content/computer", headers=headers)
        ...
        for batch in tqdm(batcher(filtered_dataset, batch_size), total=total_batches, desc="Indexing FRAMES KB"):
            ...
            try:
                response = requests.patch(f"{RIDGE_URL}/api/content?client=eval", headers=headers, files=files)
                response.raise_for_status()
...
def get_agent_response(prompt: str) -> Dict[str, Any]:
    ...
    if not is_none_or_empty(RIDGE_API_KEY):
        headers["Authorization"] = f"Bearer {RIDGE_API_KEY}"
...
def evaluate_response_for_ir(
    ...
    """Evaluate Ridge response against benchmark ground truth using string matching"""
    try:
