...
# ----------------------------------------------------------------------------------------------------
@pytest.mark.django_db
def test_get_org_files_with_org_suffixed_dir_doesnt_raise_error(tmp_path, default_user: RidgeUser):
    # Arrange
...
def test_text_search_setup_with_empty_file_creates_no_entries(
    org_config_with_only_new_file: LocalOrgConfig, default_user: RidgeUser
):
    # Arrange
...
def test_text_indexer_deletes_embedding_before_regenerate(
    content_config: ContentConfig, default_user: RidgeUser, caplog
):
    # Arrange
    existing_entries = Entry.objects.filter(user=default_user).count()
...
# ----------------------------------------------------------------------------------------------------
@pytest.mark.django_db
def test_text_index_same_if_content_unchanged(content_config: ContentConfig, default_user: RidgeUser, caplog):
    # Arrange
...
async def test_text_search(search_config: SearchConfig):
    # Arrange
    default_user = await RidgeUser.objects.acreate(
...
# ----------------------------------------------------------------------------------------------------
@pytest.mark.django_db
def test_entry_chunking_by_max_tokens(org_config_with_only_new_file: LocalOrgConfig, default_user: RidgeUser, caplog):
    # Arrange
...
def test_entry_chunking_by_max_tokens_not_full_corpus(
    org_config_with_only_new_file: LocalOrgConfig, default_user: RidgeUser, caplog
):
    # Arrange
    # Insert org-mode entry with size exceeding max token limit to new org file
    data = {
...
# ----------------------------------------------------------------------------------------------------
@pytest.mark.django_db
def test_regenerate_index_with_new_entry(content_config: ContentConfig, new_org_file: Path, default_user: RidgeUser):
    # Arrange
...
# ----------------------------------------------------------------------------------------------------
@pytest.mark.django_db
def test_update_index_with_deleted_entry(org_config_with_only_new_file: LocalOrgConfig, default_user: RidgeUser):
    # Arrange
    existing_entries = list(Entry.objects.filter(user=default_user).values_list("compiled", flat=True))
    new_file_to_index = Path(org_config_with_only_new_file.input_files[0])
...
# ----------------------------------------------------------------------------------------------------
@pytest.mark.django_db
def test_update_index_with_new_entry(content_config: ContentConfig, new_org_file: Path, default_user: RidgeUser):
    # Arrange
    existing_entries = list(Entry.objects.filter(user=default_user).values_list("compiled", flat=True))
    org_config = LocalOrgConfig.objects.filter(user=default_user).first()
    data = get_org_files(org_config)
...
def test_update_index_with_deleted_file(
    org_config_with_only_new_file: LocalOrgConfig, text_to_entries: TextToEntries, default_user: RidgeUser
):
...
def test_text_search_setup_github(content_config: ContentConfig, default_user: RidgeUser):
    # Arrange
