...
def search_config() -> SearchConfig:
    ...
    model_dir = resolve_absolute_path("~/.ridge/search")
...
def default_user4():
    ...
    """
    if RidgeUser.objects.filter(username="default4").exists():
        return RidgeUser.objects.get(username="default4")
...
def api_user(default_user):
    if RidgeApiUser.objects.filter(user=default_user).exists():
        return RidgeApiUser.objects.get(user=default_user)

    return RidgeApiUser.objects.create(
        user=default_user,
        name="api-key",
        token="kk-secret",
    )
...
def api_user2(default_user2):
    if RidgeApiUser.objects.filter(user=default_user2).exists():
        return RidgeApiUser.objects.get(user=default_user2)
...
def api_user3(default_user3):
    if RidgeApiUser.objects.filter(user=default_user3).exists():
        return RidgeApiUser.objects.get(user=default_user3)
...
def api_user4(default_user4):
    if RidgeApiUser.objects.filter(user=default_user4).exists():
        return RidgeApiUser.objects.get(user=default_user4)
...
def content_config(tmp_path_factory, search_models: SearchModels, default_user: RidgeUser):
...
@pytest.fixture(scope="function")
def chat_client(search_config: SearchConfig, default_user2: RidgeUser):
    return chat_client_builder(search_config, default_user2, require_auth=False)


@pytest.fixture(scope="function")
def chat_client_with_auth(search_config: SearchConfig, default_user2: RidgeUser):
    return chat_client_builder(search_config, default_user2, require_auth=True)


@pytest.fixture(scope="function")
def chat_client_no_background(search_config: SearchConfig, default_user2: RidgeUser):
...
@pytest.fixture(scope="function")
def client_offline_chat(search_config: SearchConfig, default_user2: RidgeUser):
    # Initialize app state
    state.config.search_type = search_config
    state.SearchType = configure_search_types()
...
@pytest.fixture(scope="function")
def new_org_file(default_user: RidgeUser, content_config: ContentConfig):
    # Setup
...
@pytest.fixture(scope="function")
def org_config_with_only_new_file(new_org_file: Path, default_user: RidgeUser):
...
@pytest.fixture(scope="function")
def pdf_configured_user1(default_user: RidgeUser):
...
All data is processed locally. User can interface with ridge app via [Emacs](./interface/emacs/ridge.el), API or Commandline
...
All data is processed locally. User can interface with ridge app via Emacs, API or Commandline
