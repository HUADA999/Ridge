...
def test_search_for_user2_returns_empty(client, api_user2: RidgeApiUser):
...
# ----------------------------------------------------------------------------------------------------
@pytest.mark.django_db(transaction=True)
def test_index_update_with_user2(client, api_user2: RidgeApiUser):
    # Arrange
...
@pytest.mark.django_db(transaction=True)
def test_index_update_with_user2_inaccessible_user1(client, api_user2: RidgeApiUser, api_user: RidgeApiUser):
    # Arrange
    files = get_sample_files_data()
    source_file_symbol = set([f[1][0] for f in files])
